...
public class DeviceTypeToIconConverter : IValueConverter, IMultiValueConverter
{
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        else if (value is DeviceViewModel vm)
        {
            ...
            isMuted = vm.Controls.Any(c => c.DisplayName.Equals("Mute", StringComparison.OrdinalIgnoreCase) && c.Value > 0.5);
        }
        ...
        return GetIconGeometry(type, isDefault, isMuted);
    }
    ...
    public object? Convert(object[]? values, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        {
            if (values[0] is DeviceType t) type = t;
            if (values[1] is bool def) isDefault = def;
            if (values[2] is bool muted) isMuted = muted;
        }
    ...
    private Geometry GetIconGeometry(DeviceType type, bool isDefault, bool isMuted)
    {
        ...
        else if (type == DeviceType.Microphone)
        {
            if (isDefault && !isMuted)
            {
                // Microphone On (no strikethrough)
                return Geometry.Parse("M12,2A3,3 0 0,1 15,5V11A3,3 0 0,1 12,14A3,3 0 0,1 9,11V5A3,3 0 0,1 12,2M19,11C19,14.53 16.39,17.44 13,17.93V21H11V17.93C7.61,17.44 5,14.53 5,11H7A5,5 0 0,0 12,16A5,5 0 0,0 17,11H19Z");
            }
            else
            {
                // Microphone Off/Inactive (with diagonal strikethrough)
                return Geometry.Parse("M12,2A3,3 0 0,1 15,5V11A3,3 0 0,1 12,14A3,3 0 0,1 9,11V5A3,3 0 0,1 12,2M19,11C19,14.53 16.39,17.44 13,17.93V21H11V17.93C7.61,17.44 5,14.53 5,11H7A5,5 0 0,0 12,16A5,5 0 0,0 17,11H19Z M3,4.5 L4.5,3 L21,19.5 L19.5,21 Z");
            }
        }

        return type switch
        {
            DeviceType.Mouse => Geometry.Parse("M12,2A6,6 0 0,0 6,8V16A6,6 0 0,0 12,22A6,6 0 0,0 18,16V8A6,6 0 0,0 12,2"),
            ...
            DeviceType.Microphone => Geometry.Parse("M12,2A3,3 0 0,1 15,5V11A3,3 0 0,1 12,14A3,3 0 0,1 9,11V5A3,3 0 0,1 12,2M19,11C19,14.53 16.39,17.44 13,17.93V21H11V17.93C7.61,17.44 5,14.53 5,11H7A5,5 0 0,0 12,16A5,5 0 0,0 17,11H19Z"),
